...
func TestValidateBuilderSubmissionV3(t *testing.T) {
	...
	forkTime := blocks[len(blocks)-1].Time() + 5
	genesis.Config.ShanghaiTime = &forkTime
	genesis.Config.CancunTime = &forkTime
	...
	parentHeader := ethservice.BlockChain().CurrentHeader()

	statedb, _ := ethservice.BlockChain().StateAt(parentHeader.Root)
	...
	cc, err := types.SignTx(types.NewTx(&types.LegacyTx{
		Nonce:    nonce,
		Value:    new(big.Int),
		Gas:      1000000,
		GasPrice: big.NewInt(2 * params.InitialBaseFee),
	...
	baseFee := eip1559.CalcBaseFee(params.AllEthashProtocolChanges, parentHeader)
	...
	blobTx := &types.BlobTx{
		...
		Nonce:      nonce,
		GasTipCap:  uint256.MustFromBig(baseFee),
	...
	tx3, err := types.SignTx(types.NewTx(blobTx), signer, testKey)

	paymentAmount := 100_000_000
	xfer, err := types.SignTx(types.NewTx(&types.DynamicFeeTx{
		...
		Value:      big.NewInt(int64(paymentAmount)),
		Data:       nil,
		AccessList: nil,
	}), signer, testBuilderKey)

	txs := []*types.Transaction{tx1, cc, tx2, tx3, xfer}
	...
	buildBlockArgs := buildBlockArgs{
		...
		feeRecipient:          testBuilderAddr,
		txs:                   txs,
		random:                common.Hash{},
		number:                parentHeader.Number.Uint64() + 1,
		gasLimit:              parentHeader.GasLimit,
		timestamp:             parentHeader.Time + 5,
		extraData:             nil,
	...
	require.EqualValues(t, len(execData.Transactions), len(txs))
	...
	reqCommitments := []deneb.KZGCommitment{deneb.KZGCommitment(commitment)}
	reqProofs := []deneb.KZGProof{deneb.KZGProof(proof)}
	reqBlobs := []deneb.Blob{deneb.Blob(blob)}
	...
	blockRequest := &BuilderBlockValidationRequestV3{
		SubmitBlockRequest: builderApiDeneb.SubmitBlockRequest{
			...
			Message: &builderApiV1.BidTrace{
				...
				Value: uint256.NewInt(uint64(paymentAmount)),
			},
			ExecutionPayload: payload,
			BlobsBundle: &builderApiDeneb.BlobsBundle{
				Commitments: reqCommitments,
				Proofs:      reqProofs,
				Blobs:       reqBlobs,
			},
		},
	...
	blockRequest.Message.Value = uint256.NewInt(uint64(paymentAmount + 1))
	...
	require.ErrorContains(t, api.ValidateBuilderSubmissionV3(blockRequest), "incorrect GasUsed 10, expected 140996")
	...
	updatePayloadHashV3(t, blockRequest)
	require.ErrorContains(t, api.ValidateBuilderSubmissionV3(blockRequest), "could not apply tx 5", "insufficient funds for gas * price + value")
}
...
func buildBlock(args buildBlockArgs, chain *core.BlockChain) (*engine.ExecutableData, error) {
	...
	if chain.Config().IsCancun(new(big.Int).SetUint64(args.number), args.timestamp) {
		blobs := uint64(0)
		for i, tx := range args.txs {
			if tx.Type() == types.BlobTxType {
				blobs += uint64(len(tx.BlobHashes()))
				sidecars = append(sidecars, tx.BlobTxSidecar())
				// remove the sidecar before we add it to the block
				args.txs[i] = tx.WithoutBlobTxSidecar()
			}
		}
		bgu := blobs * params.BlobTxBlobGasPerBlob
		blobGasUsed = &bgu
	}
	header := &types.Header{
		...
		ParentBeaconRoot: args.parentBeaconBlockRoot,
	}
	if header.BlobGasUsed != nil {
		// Assume this is the first cancun block
	...
	vmenv := vm.NewEVM(context, vm.TxContext{}, statedb, config, vmConfig)
	if beaconRoot := args.parentBeaconBlockRoot; beaconRoot != nil {
	...
	for i, tx := range args.txs {
		...
		receipt, err := core.ApplyTransaction(chain.Config(), chain, &args.feeRecipient, &gasPool, statedb, header, tx, &header.GasUsed, vmConfig)
		if err != nil {
			return nil, err
		}
	...
	block, err := chain.Engine().FinalizeAndAssemble(chain, header, statedb, &body, receipts)
	if err != nil {
		return nil, err
	}

	execData := engine.BlockToExecutableData(block, common.Big0, sidecars)
