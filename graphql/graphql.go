...
func (b *Block) resolveReceipts(ctx context.Context) ([]*types.Receipt, error) {
	if b.receipts == nil {
		...
		receipts, err := b.backend.GetReceipts(ctx, b.hash)
		if err != nil {
			return nil, err
		}
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := ethapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, gas, failed, err := ethapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	status := hexutil.Uint64(1)
	if failed {
		status = 0
	}
